...
func NewRouter(cfg config.Config, cache services.Cache, py *services.PythonClient) http.Handler {
	...
	mux.HandleFunc("/api/v1/portfolio/holdings", api.PortfolioHoldings)
	...
	mux.HandleFunc("/api/v1/portfolio/stream", api.StreamPortfolio)
	mux.HandleFunc("/api/v1/quotes/stream", api.StreamQuotes)
